...
func main() {
	...
	for slot := fromSlot; slot <= toSlot+maxInclusionDelay; slot++ {
		...
		g.Go(func() error {
			...
			root, err := bl.Capella.Message.HashTreeRoot()
			if err != nil {
				return err
			}
			bl.Capella.Message.Body.ExecutionPayload = nil // Free some memory. We don't need the payload.
			messyBlocks = append(messyBlocks, blockWithRoot{root, bl.Capella})
			return nil
		})
	}
	err = g.Wait().ErrorOrNil()
